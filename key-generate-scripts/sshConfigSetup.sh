...
append_host_config "microtik" "$IP_MICROTIK" "$SSH_USER" "${key_paths[microtik]%% *}" "${key_paths[microtik]##* }"
append_host_config "external-kali" "$IP_EXTERNAL_KALI" "$SSH_USER" "${key_paths[external-kali]%% *}" "${key_paths[external-kali]##* }"
append_host_config "shell-ftp" "$IP_SHELL_FTP" "$SSH_USER" "${key_paths[shell-ftp]%% *}" "${key_paths[shell-ftp]##* }"
append_host_config "internal-kali" "$IP_INTERNAL_KALI" "$SSH_USER" "${key_paths[internal-kali]%% *}" "${key_paths[internal-kali]##* }"
append_host_config "web-server" "$IP_WEB_SERVER" "$SSH_USER" "${key_paths[web-server]%% *}" "${key_paths[web-server]##* }"
append_host_config "database-server" "$IP_DATABASE_SERVER" "$SSH_USER" "${key_paths[database-server]%% *}" "${key_paths[database-server]##* }"
append_host_config "dns-server" "$IP_DNS_SERVER" "$SSH_USER" "${key_paths[dns-server]%% *}" "${key_paths[dns-server]##* }"
append_host_config "backup-server" "$IP_BACKUP_SERVER" "$SSH_USER" "${key_paths[backup-server]%% *}" "${key_paths[backup-server]##* }"
